...
bool SimTransmitter::initialize() {
    ...
    if (!client.is_connected()) {
        ...
        diagnostics_logger_->info("initialized", {{"tcp_connected", true}});
    }
...
void SimTransmitter::send(VelocityCommand command) {
    ...
    if (!client.is_connected()) {
        diagnostics_logger_->warning("send_failed", {{"reason", "TCP not connected"}});
        return;
    }
    ...
    if (client.send_velocity_command(command)) {
        ...
        diagnostics_logger_->warning("send_velocity_failed", {{"linear_x", command.linear_x},
                                                              {"linear_y", command.linear_y},
                                                              {"angular_z", command.angular_z}});
    }
}
...
bool SimTransmitter::is_connected() const { return SimTcpClient::instance().is_connected(); }

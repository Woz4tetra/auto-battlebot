...
namespace auto_battlebot
{
    ...
    bool SyncSocket::ensure_connected()
    {
        if (is_connected())
        {
            return true;
        ...
        // Schedule next attempt before we try (prevents busy loop when Unity is down)
        next_reconnect_attempt_ = now + std::chrono::milliseconds(config_.reconnect_interval_ms);

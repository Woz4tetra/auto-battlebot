 ...
 * Copyright (c) 2024-2025, e-con Systems.  All rights reserved.
 *
...
static int cam_power_put(struct cam *priv)
{
	...
	gpio_free(priv->pdata->pwdn_gpio);
	gpio_free(priv->pdata->reset_gpio);
...
static int cam_s_frame_interval(struct v4l2_subdev *sd, struct v4l2_subdev_frame_interval *ival)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct camera_common_data *s_data = to_camera_common_data(&client->dev);
	struct cam *priv = (struct cam *)s_data->priv;
	int ret = 0, err = 0, retry = 10;
	if (!priv || !priv->pdata) {
		return -EINVAL;
	}
...
static struct v4l2_subdev_video_ops cam_subdev_video_ops = {
	...
	.s_frame_interval = cam_s_frame_interval,
};
	...
	if (format->which == V4L2_SUBDEV_FORMAT_TRY) {
		...
		while (--retry >= 0) {
		  	err = mcu_stream_config(client, priv->format_fourcc, priv->frmfmt_mode,
					priv->frate_index);
			if (err < 0) {
			  	dev_err(&client->dev, "%s: Stream_config failure. retrying \n", __func__);
				continue;
			} else {
			  	break;
			}
				...
				ISP_CTRL_INFO * mcu_ctrl)
{
	...
	//if(mcu_ctrl->ctrl_id == V4L2_CID_FRAME_SYNC ) 
	//	return 0;
			...
			const struct i2c_device_id *id)
{
	...
	printk("RESET = %x \n",reset_gpio);
	if(reset_gpio < 0) {
		dev_err(&client->dev, "Unable to toggle RESET GPIO\n");
		return -EINVAL;
	}
	...
	printk("BOOT = %x \n",pwdn_gpio);
	if(pwdn_gpio < 0) {
		dev_err(&client->dev, "Unable to toggle BOOT GPIO\n");
		return -EINVAL;
	}
#if 0
	...
	priv =
	    devm_kzalloc(&client->dev,
			 sizeof(struct cam) +
			 sizeof(struct v4l2_ctrl *) * AR0234_NUM_CONTROLS /**num_ctrls*/,
			 GFP_KERNEL);
	if (!priv)
		return -ENOMEM;
	...
	while (--retry >= 0) {
		if (mcu_cam_init(client) < 0) {
			dev_err(&client->dev, "INIT ISP failure. retrying\n");
			continue;
		} else {
		  	break;
		}
	...
	while (--retry >= 0) {
	  	...
	  	if (err != 0) {
	    		dev_err(&client->dev, "Stream_Off failure. retrying \n");
	    		continue;
	  	} else {
	    		break;
	  	}
	...
	if(priv->mipi_lane_config == 2) { 
	common_data->def_width = AR0234_DEFAULT_WIDTH_2LANE;
	common_data->def_height = AR0234_DEFAULT_HEIGHT_2LANE;
	} else {
	common_data->def_width = AR0234_DEFAULT_WIDTH_4LANE;
	common_data->def_height = AR0234_DEFAULT_HEIGHT_4LANE;
	}

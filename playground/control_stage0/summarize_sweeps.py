...
docs/experiments/control_improvement/stage4_plant_backed_nav_plan.md.
    ...
    python playground/control_stage0/summarize_sweeps.py \
        playground/control_stage0/sweep_out --tag step1
"""
...
SCENARIO_COLUMNS: dict[str, list[str]] = {
    ...
    "ram": [
        "navigation.attack_terminal_speed_fraction",
        "impact_speed_mps",
        "min_dist_m",
        "t_contact_s",
    ],
...
def main() -> None:
    ...
    for csv in results:
        df = pd.read_csv(csv)
        cols = ["name"] + [
            c for c in SCENARIO_COLUMNS[scenario_of(csv.parent.name)] if c in df.columns
        ]

  ...
  if (cmd == cmd_DeviceInitFlash) {
...
#endif
    O_Param_Len = 0x04;
    if (param < esc_pin_count) {
      SwitchSerialOutputPin(param);
    ...
    if (param < esc_pin_count) {
      ...
      if (Enable4Way) {
        ...
        pinMode(esc_pins[esc_pin_index], OUTPUT);
        digitalWrite(esc_pins[esc_pin_index], LOW);
        delay(300);
        digitalWrite(esc_pins[esc_pin_index], HIGH);

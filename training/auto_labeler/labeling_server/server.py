...
def create_app(config: ServerConfig) -> Flask:
    ...
    def preview_mask():
        ...
        frame = video_handler.get_frame(frame_idx)
        if all_masks:
            frame = video_handler.overlay_masks(frame, all_masks, color_map, config.mask_alpha)
        ...
        for label in config.object_labels:
            ...
            if points:
                ...
                frame = video_handler.draw_points(frame, pts, lbls, label.id, tuple(label.color))
        ...
        try:
            ...
            for fid, masks in segments.items():
                for obj_id, mask in masks.items():
                    annotation_manager.set_mask(fid, obj_id, mask, propagated_from=frame_idx)

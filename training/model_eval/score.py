...
(mean pixel error and PCK@0.1 of the GT box's longer side) over IoU-matched boxes.
...
def compute_pr(frames: list[Frame], taxonomy: Taxonomy, level: str, iou_threshold: float) -> dict:
    """Precision/recall/F1 at the as-recorded confidence threshold, plus confusion counts."""
...
def compute_keypoint_metrics(frames: list[Frame], iou_threshold: float) -> dict[str, float]:
    """Keypoint error over IoU-matched box pairs where both sides carry keypoints.

    Mean pixel error and PCK: fraction of keypoints within PCK_FRACTION of the GT
    box's longer side. Label-level independent (matching is class-blind).
    Empty when the dataset or candidate has no keypoints."""
    errors: list[float] = []
    correct = 0
    for frame in frames:
        for g, p in match_indices(frame, iou_threshold):
    ...
    if not errors:
        return {}
    return {
        "kp_err_px": float(np.mean(errors)),
        f"kp_pck@{PCK_FRACTION:g}": correct / len(errors),
    }
...
def score_candidate(
    ...
    keypoint_metrics = compute_keypoint_metrics(frames, args.iou)
    rows = []
    for level in LEVELS:
        pr = compute_pr(frames, taxonomy, level, args.iou)
        confusion = pr.pop("confusion")
        row = {
            "candidate": name,
            "level": level,
            **pr,
...
def main() -> None:
    ...
    for name, mcap_path in parse_candidates(args.candidate).items():
        ...
        rows.extend(score_candidate(name, mcap_path, gt_frames, taxonomy, args))
    ...
    print(f"Wrote {args.output}/summary.csv, headline.png, confusion_*.png")

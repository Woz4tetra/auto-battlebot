...
def analyze(path: Path) -> None:
    ...
    # Collect all (timestamp_float, is_auto) samples within SVO windows.
    # We need all of them before we can determine the match window boundaries.
    ...
    # Determine match boundaries from the full sample list.
    ...
    match_samples = [
        (t, a) for t, a in raw
        if first_auto_ts is not None and match_end is not None

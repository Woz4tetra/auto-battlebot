...
std::shared_ptr<FieldDescriptionWithInlierPoints> PointCloudFieldFilter::compute_field(
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr field_cloud =
        create_point_cloud_from_depth(masked_depth_image, camera_data.camera_info.intrinsics);

    std::vector<int> inlier_indices;
    Eigen::Vector4f plane_coefficients =
        fit_plane_ransac(field_cloud, distance_threshold_, &inlier_indices);
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr inlier_cloud =
        extract_inliers(field_cloud, &inlier_indices);
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr flattened_cloud =
        transform_points(inlier_cloud, plane_transform.inverse());
    ...
    Eigen::Matrix4d flat_transform = transform_from_position_and_euler(
        Eigen::Vector3f(rectangle_centroid[0], rectangle_centroid[1], 0.0), 0.0, 0.0,
        rectangle_angle);
    ...
    diagnostics_logger_->debug(
        "find_largest_contour_mask",
        {{"original_mask_size", std::to_string(field_mask.mask.mask.rows) + "x" +
                                    std::to_string(field_mask.mask.mask.cols)},
    ...
    diagnostics_logger_->debug("create_point_cloud_from_depth",
                               {{"num_points", std::to_string(field_cloud->points.size())}});
    diagnostics_logger_->debug(
        "field_description", {{"field_width", std::to_string(field_size.x)},
    ...
    diagnostics_logger_->debug("fit_plane_ransac",
                               {{"num_inliers", std::to_string(inlier_indices.size())},
    ...
    diagnostics_logger_->debug("plane_properties",
                               {{"plane_normal_x", std::to_string(plane_normal[0])},
    ...
    diagnostics_logger_->debug("find_minimum_rectangle",
                               {{"num_corners", std::to_string(rectangle_corners.size())}});
    diagnostics_logger_->debug("rectangle_properties",
                               {{"width", std::to_string(rectangle_extents[0])},
...
FieldDescription PointCloudFieldFilter::track_field(
    TransformStamped tf_visodom_from_camera,
    std::shared_ptr<FieldDescriptionWithInlierPoints> initial_description) {
    ...
    Eigen::MatrixXd tf_cameraworld_from_camera =
        tf_visodom_from_cameraworld_.transform.tf.inverse() * tf_visodom_from_camera.transform.tf;
...
cv::Mat PointCloudFieldFilter::find_largest_contour_mask(const cv::Mat &mask) const {
    ...
    diagnostics_logger_->debug(
        {{"original_mask_size", std::to_string(mask.cols) + "x" + std::to_string(mask.rows)},
...
cv::Mat PointCloudFieldFilter::mask_depth_image(const cv::Mat &depth_image,
...
pcl::PointCloud<pcl::PointXYZ>::Ptr PointCloudFieldFilter::create_point_cloud_from_depth(
    const cv::Mat &depth_image, const cv::Mat &intrinsics) const {
...
Eigen::Vector4f PointCloudFieldFilter::fit_plane_ransac(
    const pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud, double distance_threshold,
...
Eigen::Vector3f PointCloudFieldFilter::plane_normal_from_coefficients(
    const Eigen::Vector4f &coefficients) const {
...
Eigen::Matrix4d PointCloudFieldFilter::transform_from_plane(
    const Eigen::Vector3f &plane_center, const Eigen::Vector3f &plane_normal,
...
Eigen::Matrix4d PointCloudFieldFilter::transform_from_position_and_euler(
    const Eigen::Vector3f &position, double roll, double pitch, double yaw) const {
    // Create identity transform matrix
    Eigen::Matrix4d transform = Eigen::Matrix4d::Identity();
...
pcl::PointCloud<pcl::PointXYZ>::Ptr PointCloudFieldFilter::extract_inliers(
    const pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud, const std::vector<int> *inliers) const {
...
pcl::PointCloud<pcl::PointXYZ>::Ptr PointCloudFieldFilter::transform_points(
    const pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud, const Eigen::Matrix4d &transform) const {
...
std::vector<Eigen::Vector2f> PointCloudFieldFilter::find_minimum_rectangle(
    ...
    std::vector<Eigen::Vector2f> box_corners = {{best_max_x, best_min_y},
...
Eigen::Vector2f PointCloudFieldFilter::get_rectangle_extents(
...
double PointCloudFieldFilter::get_rectangle_angle(
    const std::vector<Eigen::Vector2f> &rectangle) const {

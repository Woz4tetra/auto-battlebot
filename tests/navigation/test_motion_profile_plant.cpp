...
// --- effective_command: what the plant sees after its own deadzone eats the low end ---

TEST(MotionProfileEffectiveCommand, ZeroCommandStaysZero) {
    EXPECT_DOUBLE_EQ(Nav::effective_command(0.0, kDeadzoneAngLeft, kDeadzoneAngRight), 0.0);
}

TEST(MotionProfileEffectiveCommand, FullCommandMapsToFullEffect) {
    ...
    EXPECT_DOUBLE_EQ(Nav::effective_command(1.0, kDeadzoneAngLeft, kDeadzoneAngRight), 1.0);
    EXPECT_DOUBLE_EQ(Nav::effective_command(-1.0, kDeadzoneAngLeft, kDeadzoneAngRight), -1.0);
}

TEST(MotionProfileEffectiveCommand, CommandInsideDeadzoneProducesNothing) {
    ...
    EXPECT_DOUBLE_EQ(
        Nav::effective_command(kDeadzoneAngLeft * 0.5, kDeadzoneAngLeft, kDeadzoneAngRight), 0.0);
    EXPECT_DOUBLE_EQ(
        Nav::effective_command(-kDeadzoneAngRight * 0.5, kDeadzoneAngLeft, kDeadzoneAngRight), 0.0);
}

TEST(MotionProfileEffectiveCommand, DeadzoneIsPerSign) {
    ...
    const double left = Nav::effective_command(magnitude, kDeadzoneAngLeft, kDeadzoneAngRight);
    const double right = Nav::effective_command(-magnitude, kDeadzoneAngLeft, kDeadzoneAngRight);
    EXPECT_GT(left, 0.0);
    EXPECT_DOUBLE_EQ(right, 0.0);  // 0.02 is still inside the right deadzone
}

TEST(MotionProfileEffectiveCommand, IsMonotonicAndOddOnASymmetricDeadzone) {
    double previous = -1.0;
    for (double command = 0.0; command <= 1.0; command += 0.05) {
        const double effect = Nav::effective_command(command, 0.1, 0.1);
        EXPECT_GE(effect, previous);
        EXPECT_DOUBLE_EQ(Nav::effective_command(-command, 0.1, 0.1), -effect);
        previous = effect;
    }
}

TEST(MotionProfileEffectiveCommand, AbsurdDeadzoneIsClampedNotDividedByZero) {
    // Guards the 1/(1 - deadzone) denominator against a bad refit. Clamped at 0.95.
    const double effect = Nav::effective_command(1.0, 5.0, 5.0);
    EXPECT_TRUE(std::isfinite(effect));
    EXPECT_DOUBLE_EQ(effect, 1.0);
}

TEST(MotionProfileEffectiveCommand, NoDeadzoneIsIdentity) {
    for (const double command : {-1.0, -0.4, 0.0, 0.25, 1.0}) {
        EXPECT_DOUBLE_EQ(Nav::effective_command(command, 0.0, 0.0), command);
    }
}
...
TEST(MotionProfilePlantAlgebra, DeadzoneKeepsTinyTurnsFromCostingForwardSpeed) {
    ...
    const double effect = Nav::effective_command(tiny_turn, kDeadzoneAngLeft, kDeadzoneAngRight);

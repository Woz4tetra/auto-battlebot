...
class ChannelStreamingParser:
    ...
    def _parse_channel_packet_phase(self, packet_data: bytes) -> Optional[List[int]]:
        ...
        if len(channel_data) != self.channels_per_packet * 2:
            print(
                f"Invalid channel data length in phase {phase}: {len(channel_data)}, expected "
                f"{self.channels_per_packet * 2}"
            )
            return None
...
def main():
    ...
    parser.add_argument(
        "--stats-interval", type=int, default=5, help="Statistics display interval in seconds"
    )

    args = parser.parse_args()

    print("OpenTX Channel Data Streaming Test")
    ...
    try:
        ...
        if stats["packets_received"] > 0:
            corruption_rate = (
                ...
                / (stats["packets_received"] + stats["packets_corrupted"])
            ) * 100

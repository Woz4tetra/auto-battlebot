...
seg model's mask and box, the mask centroid, the box center, and the hand-labeled
keypoint front/back midpoint. The figure for `mask_centroid_vs_box_2026-08-03.md`, where
the numbers say the centroid and the box center coincide while both miss the aim point.
...
# Categorical slots 1-3 of the validated default palette, as BGR. Each estimate also
# carries a distinct marker shape, so identity never rests on color alone.
...
def draw_tile(
    ...
    """Crop around one robot and overlay the mask, box and the three position estimates."""
    side = max(box[2] - box[0], box[3] - box[1]) * CROP_MARGIN
    ...
    # Largest first: a coincident centroid stays visible on top of the box center.
...
def legend_strip(width: int) -> np.ndarray:
    ...
    entries = [
        ...
        ("bounding box center", BOXCENTER_BGR, "square"),

...
def _worker_propagate(
    ...
    print(f"[GPU {gpu_id}] Initializing inference state for frames {start_frame}-{start_frame + num_frames}...")
...
class MultiGPUTracker:
    ...
    def set_video(self, video_path: str):
        ...
        print(f"Video: {self.video_width}x{self.video_height}, {self.total_frames} frames")
        print(f"Inference scale: {self.inference_scale} -> {self.scaled_width}x{self.scaled_height}")
    ...
    def propagate_from_frame(
        ...
        for chunk in chunks:
            print(f"  GPU {chunk['gpu_id']}: frames {chunk['start_frame']}-{chunk['start_frame'] + chunk['num_frames'] - 1}")

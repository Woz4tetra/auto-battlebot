...
class LabelingUI:
    ...
    def _create_control_panel(self, parent):
        ...
        ttk.Radiobutton(
            mode_frame, text="➕ Add (Include)", 
            variable=self.mode_var, value="add",
            command=self._on_mode_change,
        ).pack(anchor='w')
        ttk.Radiobutton(
            mode_frame, text="➖ Remove (Exclude)", 
        ...
        self.propagate_btn = ttk.Button(
            prop_frame, text="▶ Propagate Masks",
        ...
        ttk.Button(
            jump_frame, text="⏪ Prev Manual",
            command=self._go_prev_manual,
        ).pack(side='left', padx=2)
        
        ttk.Button(
            jump_frame, text="Next Manual ⏩",
            command=self._go_next_manual,
        ).pack(side='left', padx=2)
        
        ttk.Button(
            nav_frame, text="🔎 Go to Next Unlabeled",
        ...
        ttk.Checkbutton(
            display_frame, text="Show Points",
    ...
    def _bind_keys(self):
        ...
        for i in range(1, 10):
            self.root.bind(str(i), lambda e, idx=i: self._select_label_by_index(idx - 1))
    ...
    def _update_display(self):
        ...
        try:
            ...
            resized = self._current_image.resize((new_w, new_h), Image.Resampling.LANCZOS)

...
double SafestPointTarget::circle_radius(double x, double y, const FieldDescription &field,
    ...
    return empty_circle_radius(field.size.size, x, y, opponents, field.hazards);
}
...
std::optional<Pose2D> SafestPointTarget::solve(const FieldDescription &field,
    ...
    const EmptyCircle best = solve_exact(field.size.size, opponents, field.hazards, our_pose);
    ...
    logger_->debug("solver", {{"x", best.center.x},
                              ...
                              {"n_opponents", static_cast<int>(opponents.size())},
                              {"n_hazards", static_cast<int>(field.hazards.size())},

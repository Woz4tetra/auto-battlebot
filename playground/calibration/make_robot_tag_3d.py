...
This is the 3D-print counterpart of make_print_tags.py for the tag that mounts on the robot top. Instead of
a paper tag you tape down, it emits a printable plate: a white base with the black AprilTag cells raised on
top as a second material. Two watertight objects share one coordinate frame, so in Bambu Studio you assign
white to the base and black to the tag and slice.

The black geometry comes straight from the cv2.aruco DICT_APRILTAG_36h11 bit pattern, so the printed tag is
geometrically identical to what apriltag_track.py detects (id and orientation verified by re-detecting a
top-down raster before writing). A white margin around the tag gives the quiet zone the detector needs.

The 3MF is written directly (a zip of XML) so it needs no lxml: two named, pre-colored objects that import
into Bambu Studio already split into a white base and a black tag. STL pairs are written too.
...
Default: id 0, 130 mm tag, 2 mm white base, 0.6 mm raised black, 16 mm white margin (a ~1-cell quiet zone).
The plate is 162 x 162 mm, well within a 256 mm Bambu bed. Bambu Studio: import robot_tag.3mf, confirm the
tag object is black filament and the base is white, then slice. Use 0.1-0.2 mm layers and a base thickness
that is a clean multiple of the layer height. Note its physical edge (--tag-size) and heading offset for
apriltag_track.py --tag-size / --yaw-offset-deg.
"""
...
def black_cell_boxes(
    origin: np.ndarray, tag_mm: float, grid: np.ndarray, base_h: float, tag_h: float,
) -> list[trimesh.Trimesh]:
    """Boxes for the black cells, raised on top of the base. origin is the tag's (x0, y0) corner in mm.

    Consecutive black cells in a row are merged into one box to keep the mesh small and free of internal
    coplanar faces.
    """
    side = grid.shape[0]
    cell = tag_mm / side
...
def verify(grid: np.ndarray, tag_id: int, tag_mm: float, ppm: float) -> None:
    """Rasterize the black cells top-down with a quiet zone and re-detect, confirming id and orientation."""
    side = grid.shape[0]
    cell = tag_mm / side
    pad = tag_mm * 0.25
...
def main() -> None:
    ...
    p.add_argument("--margin", type=float, default=10.0, help="white quiet-zone border around the tag (mm)")
    args = p.parse_args()
    ...
    print(f"robot tag: id {args.tag_id}, {tag_mm:.0f} mm tag on a {plate:.0f} x {plate:.0f} mm plate, "
    ...
    print(f"\nWrote to {args.out_dir}. Import robot_tag.3mf into Bambu Studio (it imports as a white base "

...
YoloSegRobotBlobModel::YoloSegRobotBlobModel(YoloSegRobotBlobModelConfiguration &config)
    ...
    diagnostics_logger_ = DiagnosticsLogger::get_logger("yolo_seg_robot_blob_model");
}

bool YoloSegRobotBlobModel::initialize() {
...
float YoloSegRobotBlobModel::letterbox(cv::Mat &input_image, cv::Mat &output_image,
...
std::vector<YoloSegRobotBlobModel::Detection> YoloSegRobotBlobModel::decode_detections(
    ...
    int feature_dim = static_cast<int>(det_shape[1]);
    int num_predictions = static_cast<int>(det_shape[2]);
...
float YoloSegRobotBlobModel::iou(const Detection &lhs, const Detection &rhs) {
...
void YoloSegRobotBlobModel::append_detection_keypoints(const Detection &det,
    ...
    if (!instance_mask.empty()) {
        midline = find_longest_rectangle_midline(instance_mask);
    }
    if (!midline.valid) {
        ...
        midline = bbox_to_midline(x1, y1, x2, y2);
    }
